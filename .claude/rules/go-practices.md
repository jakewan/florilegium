...
- `errcheck` runs with `check-blank: true`, so discarding an error to `_` is itself a lint failure — `_ = f()` does not silence an unwanted error. Capture and inspect it, fold a secondary cleanup error into the primary one with `errors.Join(primary, os.Remove(tmp))`, or when both a read and a Close can fail prefer the read error (as `recent` does). The point is to act on every error, not to suppress it.
...
- Smoke-testing the running binary over stdio needs a driver that holds stdin open until each reply is read. Piping a batch of requests and letting stdin close (`printf … | florilegium`) races the EOF shutdown — the session tears down before responses flush, so the binary exits 0 with no output, a false pass. Prefer the in-memory transport for automated coverage (see Tests); for a manual end-to-end check — e.g. confirming a shipped corpus loads and serves — drive the binary from a harness (a short script speaking newline-delimited JSON-RPC) that reads each response before sending the next and closes stdin last.
