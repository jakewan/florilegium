...
- **Generic examples are fine.** Illustrative corpus items, placeholder ids, and example config that carry no real personal data are encouraged.
- When an example needs a path or name, use an obvious placeholder (`/path/to/corpus.yml`, `example.com`, `your-name`) rather than a real one.
